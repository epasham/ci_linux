...
if [[ "$originalArgOne" == mongo* ]] && [ "$(id -u)" = '0' ]; then
	if [ "$originalArgOne" = 'mongod' ]; then
		chown -R mongodb:mongodb /data/configdb /data/db /data/log
	fi
	...
	# ./install.sh
	#fi
	chown -R netbrain:netbrain /data/license

    if [[ $@ = *"--sslMode requireSSL"* ]]; then
       su netbrain -c "/usr/bin/netbrainlicense/licensed -f /usr/licensed/licensed_ssl.conf"
    else
